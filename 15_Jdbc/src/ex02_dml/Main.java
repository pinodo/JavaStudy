...
// 2. 미리 쿼리문을 컴파일해 둔 뒤, 실행 직전 필요한 값을 전달하는 방식을 사용
...
public class Main {
  ...
  public static void insert() throws Exception {
    ...
    // 파라미터 바인딩 (?에 값 채우기): 채울 때 문자열은 작은 따옴표로 자동으로 감쌈
    ps.setString(1, "QA");
    ps.setString(2, "Incheon");

    // 퀴리문 실행 (실행 결과는 int 타입)
  ...
  public static void update() {
    ...
    try {
      ...
      try {
        if (conn != null) {
          conn.setAutoCommit(true); // 기본 세팅으로 변경 -> 커넥션 반납 후에도, 풀에 살아있음
          conn.close();
        }
